...
DS1307_TIME DS1307_GetTime(DS1307_Handle* handle) {
	...
	if (DS1307_WaitUntilReady(handle) == DS1307_RES_OK) {
		...
		if (HAL_I2C_Master_Transmit(handle->i2c, handle->DS1307_ADDRESS +1, &a, 1,
				handle->TIMEOUT)) {
			return time;
		}
...
DS1307_TIME MyDS1307_GetTime(I2C_HandleTypeDef *hi2c) {
		...
		HAL_I2C_Master_Transmit(hi2c,0xD0,dataTemp,1,HAL_MAX_DELAY);
		HAL_I2C_Master_Receive(hi2c,0xD1,data,8,HAL_MAX_DELAY); // 0xD0 or 0xD1 both ok for Receive
		time.second += data[0] & 0xF; //(1) if time.second not assign 0 => time.second = data[0] & 0xF,(2) and not need to use += 
		time.second += 10 * ((data[0] & 0x70) >> 4);
...
void MyDS1307_SetTime(I2C_HandleTypeDef *hi2c, DS1307_TIME time) {
		...
		HAL_I2C_Master_Transmit(hi2c, 0xD0, data,8, HAL_MAX_DELAY);
